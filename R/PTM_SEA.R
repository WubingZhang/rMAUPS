#' Single sample GSEA
#' @name ssGSEA2
#' @param inputMat A matrix with rownames matching the terms in ptm signatures.
#' @param gene.set.databases List of genesets (in gmt format) to evaluate enrichment on.
#' @param ptmsig.type The type of phosphorylation site signature, One of "uniprot", "sitegrpid", or "flanking".
#' @param ptmsig.org Organism, "human", "mouse", or "rat".
#' @param sample.norm.type Sample normalization, c("rank", "log", "log.rank", "none").
...
#' @param statistic Method for testing statistic, c("area.under.RES", "Kolmogorov-Smirnov").
#' @param output.score.type Output score type, c("NES", "ES")
#' @param nperm Number of random permutations for NES case.
#' @param combine.mode "combine.off" do not combine *_UP and *_DN versions in a single score.
...
#' @param min.overlap Minimum overlap genes or phosphorylation sites.
#' @param correl.type Correlation type: "rank", "z.score", "symm.rank"
#' @param global.fdr If TRUE calculate global FDR; else calculate FDR sample-by-sample
#' @param extended.output If TRUE the result GCT files will contain statistics about gene coverage etc.
#' @param par Do parallel running.
#' @param spare.cores Spare cores.
#'
#' @note Original code written by Pablo Tamayo. Adapted with additional modifications
#' Referece:
...
#' @import verification doParallel foreach magrittr parallel
#' @export
#'
ssGSEA2 <- function(
  inputMat,
  gene.set.databases=NULL,
  ptmsig.type=c("uniprot", "sitegrpid", "flanking")[1],
  ptmsig.org=c("human", "mouse", "rat")[1],
  sample.norm.type=c("rank", "log", "log.rank", "none")[1],  ## sample normalization
  weight= 0,                     ## when weight==0, all genes have the same weight; if weight>0,
  ## actual values matter, and can change the resulting score
  statistic           = c("area.under.RES", "Kolmogorov-Smirnov")[1], ## test statistic
  output.score.type   = c("NES", "ES")[1],
  nperm               = 1000,    ## number of random permutations for NES case
  combine.mode        = c("combine.off", "combine.replace", "combine.add")[1],
  min.overlap         = 10,
  correl.type         = c("rank", "z.score", "symm.rank")[1],  ## correlation type: "rank", "z.score", "symm.rank"
  ...
  ## ###################################################
  if(is.null(gene.set.databases)){
    gene.set.databases = paste0(system.file("extdata", package = "rMAUPS"), "/ptmsigdb/",
                                "ptm.sig.db.all.", ptmsig.type, ".", ptmsig.org, ".v1.9.0.gmt")
    if(!file.exists(gene.set.databases)) stop("Can't find gene set database ...")
  }
  ...
  ##}
  message(Sys.time(), ' Overlap between rownames of the dataset and the each gene set.')
  ## ####################################################
  ...
  message('Testing ', length(keep.idx), ' gene sets:\n')
  ...
  if(par){
    ## register cores
    cl <- parallel::makeCluster(parallel::detectCores() - spare.cores)
    ...
    tmp <-  foreach(gs.i = 1:N.gs,  .packages='doParallel') %dopar% {
      ...
      if (output.score.type == "ES") {
        ...
        OPAM <- project.geneset (data.array = m, gene.names = gene.names, n.cols = Ns,
                                 ...
                                 gene.set = gene.overlap, nperm = 0, correl.type = correl.type,
                                 gene.set.direction = gene.set.direction, min.overlap = min.overlap,
                                 size.G.current = size.G[gs.i])

      } else if (output.score.type == "NES") {
        OPAM <- project.geneset (data.array = m, gene.names = gene.names, n.cols = Ns, n.rows= Ng,
                                 weight = weight, statistic = statistic, gene.set = gene.overlap,
                                 nperm = nperm, correl.type = correl.type, gene.set.direction = gene.set.direction,
                                 min.overlap = min.overlap, size.G.current = size.G[gs.i])
      }
      OPAM
    }
    ...
    tmp <- lapply(1:N.gs, function(gs.i){
      ...
      # message( gs.names[gs.i], '  ' )
      # message( (gs.i / N.gs)*100)
      # message(names(gs)[gs.i])
      ...
      if (output.score.type == "ES") {
        OPAM <- project.geneset (data.array = m, gene.names = gene.names, n.cols = Ns, n.rows= Ng,
                                 weight = weight, statistic = statistic, gene.set = gene.overlap,
                                 nperm = 1, correl.type = correl.type, gene.set.direction = gene.set.direction,
                                 min.overlap = min.overlap, size.G.current = size.G[gs.i])
  ...
  colnames(Signature.set.overlap) <- paste(sample.names, 'Signature.set.overlap', sep='.')
  Signature.set.overlap.size <- ol.numb.matrix.2
  colnames(Signature.set.overlap.size) <- paste(sample.names, 'Signature.set.overlap.size', sep='.')
  Signature.set.overlap.percent <- ol.perc.matrix.2
  colnames(Signature.set.overlap.percent) <- paste(sample.names, 'Signature.set.overlap.percent', sep='.')
  ...
  fdr.tmp <- data.matrix(fdr.matrix.2)
  colnames(fdr.tmp) <- paste(sample.names, 'fdr-pvalue', sep='.')
  pval.tmp <- data.matrix(pval.matrix.2)
  colnames(pval.tmp) <- paste(sample.names, 'pvalue', sep='.')
  gs.descs.2 <-  data.frame(gs.descs.2, pval.tmp, fdr.tmp, stringsAsFactors = F)

  ## generate GCT
  ALL.GCT <- new('GCT')
  ALL.GCT@mat <- data.matrix(score.matrix.2)
  rownames(ALL.GCT@mat) = gs.names.2
  colnames(ALL.GCT@mat) = sample.names
...
#'
#' @name Read.GeneSets.db2
#' @param gs.db Signature database.
#' @param thres.min Minimum size.
#' @param thres.max Maxmum size.
#'
#' @note 20161013 modified by kk
#'
...
#' Chops the string to the specified number of characters
#' @name chopString
#' @param string A number of characters.
#' @param nChar N chars.
#' @param add.dots Whether add dots into the string.
#'
#' @note Given a string and a number of characters
...
my.col2rgb <- function(color, alpha=80, maxColorValue=255){
  ...
  for(col in 1:length(color)){
    col.rgb <- grDevices::col2rgb(color[col])
    out[col] <- grDevices::rgb(col.rgb[1], col.rgb[2], col.rgb[3],
...
#' Calculate ES score
#' @param max.ES Max enrichment score.
#' @param min.ES Min enrichment score.
#' @param RES Result.
#' @param gaps Gaps
#' @param valleys Valleys
#' @param statistic Statistics for testing.
#'
score <- function(max.ES,
                  min.ES,
                  RES,
                  gaps,
                  valleys,
                  statistic = c("area.under.RES", "Kolmogorov-Smirnov")[1]){
  ## KM
...
#' @param ordered.gene.list Ordered gene list.
#' @param gene.set2 Gene sets.
#' @param weight Weight as that in ssGSEA2.
#' @param n.rows N rows.
#' @param correl.type Correlation type as that in ssGSEA2.
#' @param gene.set.direction Statistics for testing.
#' @param data.expr Expression data matrix.
#' @param statistic Statistics for testing.
...
gsea.score <- function (ordered.gene.list,
  ...
  if(!is.null(gene.set.direction)){
    ...
    gsea.results = list(ES = ES, arg.ES = arg.ES, RES = RES,
                        indicator = ind, correl.vector = correl.vector,
                        step.up=up, step.down=1/Nm)
  } ## end else
...
#' @param n.cols N colymns.
#' @param n.rows N rows.
#' @param weight Weight as that in ssGSEA2.
#' @param statistic Statistics for testing.
#' @param gene.set Gene sets or signatures.
#' @param nperm N permutation.
#' @param correl.type Correlation type.
#' @param gene.set.direction Gene set direction.
#' @param min.overlap Minimum overlap.
#' @param size.G.current Size of the current signature.
#'
project.geneset <- function (data.array,
  ...
  for (sample.index in 1:n.cols) {
    ...
    if(sum(gene.names %in% gene.set) < min.overlap){
      ...
      ## this takes care about redundant gene lists, the approach above does not return the locations of 'gene.set'
      ## in 'gene.names' but the indices of 'gene.names' present in 'gene.set'
      ...
      GSEA.results <- gsea.score (gene.list, gene.set2, weight, n.rows,
                                  correl.type, gene.set.direction, data.expr,
                                  statistic, min.overlap)
      ...
      if (nperm == 0) {
        ...
        ES.tmp = sapply(1:nperm,  function(x) gsea.score(ordered.gene.list = sample(1:n.rows),
                                                         gene.set2 = gene.set2,
                                                         weight = weight, n.rows = n.rows,
